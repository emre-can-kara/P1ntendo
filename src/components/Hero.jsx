...
function Hero() {
  ...
  return (
    <div id="home" className="relative h-[640px] w-full overflow-hidden">
      ...
      <button 
        onClick={prevSlide}
        className="absolute left-8 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-gray-800 p-4 rounded-full shadow-lg transition-all duration-300"
      >
      ...
      <button 
        onClick={nextSlide}
        className="absolute right-8 top-1/2 -translate-y-1/2 bg-white/80 hover:bg-white text-gray-800 p-4 rounded-full shadow-lg transition-all duration-300"
      >
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 flex space-x-3">
        {slides.map((_, index) => ( // hocaya sor '_'
           <button
            key={index}
